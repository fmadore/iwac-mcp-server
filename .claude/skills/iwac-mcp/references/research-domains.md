...
- **Islamic publications vs. mainstream press:** `search_publications` covers Islamic community media (Islam Info, An-Nasr Vendredi, …). Most items are entire issues — navigate by series (`list_periodicals`), subject (87% tagged), country and year; use `get_publication_fulltext` keyword excerpts to read inside an issue.
