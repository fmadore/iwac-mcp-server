...
await withFixtureScope(async (fixtures) => {
  ...
  const { client, close: closeClient } = await fixtures.connect({ name: "token-budget" });
  ...
  const { client: stress, close: closeStress } = await fixtures.connect({ name: "token-budget-stress", cacheDir: "fixtures-stress" });
