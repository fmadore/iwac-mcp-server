...
10. Use `search_audiovisual` / `list_audiovisual` for recorded speech (~1,770 items: 1,724 francophone web videos from Burkina Faso, Togo and Benin, plus 47 deposited Nigerian Hausa/Arabic recordings). Keyword search reaches each item's own description (filled for ~1,465 rows, and the main thing worth searching here), but AI summaries are empty corpus-wide and only ~50 transcriptions are public, so a keyword search reads a synopsis rather than what was actually said -- do not quote an audiovisual item as though you had heard it
