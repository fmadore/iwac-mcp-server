...
| Nigeria | 0 | No press articles at all — 45 deposited audiovisual recordings only |
...
Five models scored the corpus independently: **`gpt-5-6-luna`**, **`mistral-small-2603`**, **`deepseek-v4-flash-0731`**, **`gemma-4-31b-it`** and **`qwen3-8-27b`** (added August 2026). All five ran the same French prompt, which is what makes them comparable to each other. `get_sentiment_distribution` reaches all five (and `model="all"` reports their agreement); the inline `polarity` / `centrality` / `subjectivity` columns on search results, and `search_by_sentiment`'s filters, are `gpt-5-6-luna`'s only.

**The five do not cover the same articles, so never compare their raw counts without reading `coverage`.** Four of them score 12,298 articles on polarity and centrality — of the 12,349 that existed when the panel ran, and of 13,397 in the corpus today, the difference being the unenriched tail described in rule 9 of SKILL.md; the ~51 they skip are the Ewé / Kabiyè / Dendi / untagged ones, left out on purpose because the annotation prompt is French and a French-prompted model returns confident but unusable output for them. **`qwen3-8-27b` scores 12,098** — 200 fewer — and that gap is deliberate and final rather than a run to repair: each of those articles was attempted four times and then retired. It is also **not missing at random**: the failures concentrate on articles the panel reads as peripheral to Islam (6.2% of `Marginal` articles unscored against ~1% of `Central` and `Très central` ones), because the model declines to place a subjectivity label where Islam is marginal while the prompt licenses declining only where Islam is absent. Two consequences follow. Qwen's shortfall must not be reported as a defect to be fixed; and **any base restricted to articles all five scored leans slightly toward material where Islam is central** — which is exactly what `scored_by_all` is. Every model reports its own `coverage` block; read it before comparing counts.

**Each new annotator lowered the agreement figure, it did not confirm the earlier ones.** Unanimity on polarity is **32%** (3,929 of the 12,098 all five scored), where four models reached 36% and the first three alone reached 43%. Any earlier number in a draft that says 36%, 43% or 54% predates a panel member and should be restated, not reconciled.
...
- **Subjectivity** (objectivity of representation): Très objectif → Plutôt objectif → Mixte → Plutôt subjectif → Très subjectif. An ordinal **label**, not a number (generation 1 stored a 1-5 float; the column kept its `…_score` name, so only the values tell you which you are holding). `get_sentiment_distribution` also returns a `mean_rank` / `median_rank` derived by ranking the labels 1-5 — a position on a five-point scale, not a percentage

### Known Issues
- **Cross-cultural framing:** none of the five is trained predominantly on French-language material, still less on the French of West African newsrooms, and all may misread its conventions
...
- **Subjectivity is much the weakest of the three scales.** Pairwise κ across the five models, measured on the whole August 2026 revision, runs **0.16–0.52** — against 0.45–0.74 for centrality on the same rows — and on the 2026-07-29 pilot DeepSeek reproduced its own answer on a re-run only **47%** of the time. The 0.52 ceiling is one pair (`gpt-5-6-luna`↔`qwen3-8-27b`); every pair involving Mistral still runs 0.16–0.26. These are single-run annotations, so that variance is baked into the stored value. Report it as weak evidence, with the caveat, or not at all. Polarity and centrality are far stronger. `get_sentiment_distribution` ships the caveat in the payload; quote it
- **Never align the models by their subjectivity gaps — condition on centrality.** `gpt-5-6-luna`, `mistral-small-2603`, `gemma-4-31b-it` and `qwen3-8-27b` each omit a subjectivity label *exactly* where they said `Non abordé` — 290, 254, 243 and 287 rows respectively, on top of the articles they never annotated at all — so their gap is a principled abstention. `deepseek-v4-flash-0731` is the only one that omits beyond that: 489 rows, 18 of which it did say discuss Islam. So a row where Luna is empty and Mistral says `Très objectif` is not a disagreement about objectivity — it is the two disagreeing about whether the article concerns Islam at all
- **On centrality the Mistral family is a systematic outlier** (`mistral-small-2603` runs κ 0.45–0.53 pairwise where non-Mistral pairs run 0.66–0.74, measured full-corpus) — a persistent family reading of the construct, not a bad run. `gemma-4-31b-it` and `qwen3-8-27b` both joined the non-Mistral cluster (κ 0.71 and 0.69 with Luna, 0.74 with each other), so a 4-of-5 majority on centrality is almost always the others outvoting Mistral: describe it that way rather than as a panel consensus
- **On polarity `qwen3-8-27b` now sits closest to Luna** (κ 0.54, ahead of Gemma's 0.50), but it is also the panel's most reluctant to reach for an extreme — 176 `Très positif` and 5 `Très négatif`, against Luna's 425 and 45 — so Qwen agreeing with Luna corroborates a middle label far more than a strong one
- **`gemma-4-31b-it` reads far more articles as `Neutre`** (7,275) than Luna does (5,017), so where the two agree the reading is comparatively well founded and where Gemma alone says `Neutre` it is usually reading descriptive coverage as neutral rather than positive
- **Re-annotating does not reproduce the stored answer.** Vendor-side temperature is not zero (DeepSeek V4 runs at 1.0), so a re-run is a fresh reading, not a correction. Anything that assumes these values are reproducible on demand is wrong
...
- Pass `model="all"` before quoting any single number: where the five models split, no one model's figure should be reported alone
- `search_articles` results include `gpt-5-6-luna`'s polarity, centrality, and subjectivity inline — use these to build topic-specific sentiment tables directly from search results, naming the model
