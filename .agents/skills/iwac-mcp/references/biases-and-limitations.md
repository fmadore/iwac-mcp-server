...
Four models scored the corpus independently: **`gpt-5-6-luna`**, **`mistral-small-2603`**, **`deepseek-v4-flash-0731`** and **`gemma-4-31b-it`** (added August 2026). All four ran the same French prompt, which is what makes them comparable to each other. Coverage is near-total but not complete — 12,298 of 12,349 articles on polarity and centrality — so compare `scored_by_all` against `total_articles` rather than assuming they match. The ~51 missing articles are the Ewé / Kabiyè / Dendi / untagged ones, skipped on purpose because the annotation prompt is French and a French-prompted model returns confident but unusable output for them. `get_sentiment_distribution` reaches all four (and `model="all"` reports their agreement); the inline `polarity` / `centrality` / `subjectivity` columns on search results, and `search_by_sentiment`'s filters, are `gpt-5-6-luna`'s only.

**A fourth annotator lowered the agreement figure, it did not confirm the first three.** Unanimity on polarity is **36%** (4,488 of 12,298) across all four, where the first three alone reached 43%. Any earlier number in a draft that says 43% or 54% predates a panel member and should be restated, not reconciled.

**Always name the model behind a reported figure.** The columns name the exact model that produced them, and so do the tools; vendor shorthand (`chatgpt`, `mistral`, `deepseek`, `gemma`/`google`) is accepted as input and resolves to the model id, which is what the payload echoes back.
...
### Known Issues
- **Cross-cultural framing:** all four models are trained predominantly on English-language data and may misread French-language West African press conventions
...
- **Subjectivity is much the weakest of the three scales.** Pairwise κ across the four models, measured on the whole August 2026 revision, runs **0.16–0.47** — against 0.46–0.72 for centrality on the same rows — and on the 2026-07-29 pilot DeepSeek reproduced its own answer on a re-run only **47%** of the time. These are single-run annotations, so that variance is baked into the stored value. Report it as weak evidence, with the caveat, or not at all. Polarity and centrality are far stronger. `get_sentiment_distribution` ships the caveat in the payload; quote it
- **Never align the models by their subjectivity gaps — condition on centrality.** `gpt-5-6-luna` and `gemma-4-31b-it` each omit a subjectivity label exactly where they said `Non abordé` (341 and 294 rows, the 51 unscanned ones included); `deepseek-v4-flash-0731` omits on ~489 articles, 18 of which it did say discuss Islam. So a row where Luna is empty and Mistral says `Très objectif` is not a disagreement about objectivity — it is the two disagreeing about whether the article concerns Islam at all
- **On centrality the Mistral family is a systematic outlier** (`mistral-small-2603` runs κ 0.46–0.53 pairwise where non-Mistral pairs run 0.67–0.72, measured full-corpus) — a persistent family reading of the construct, not a bad run. `gemma-4-31b-it` joined the non-Mistral cluster (κ 0.71 with Luna), so a 3-of-4 majority on centrality is almost always the others outvoting Mistral: describe it that way rather than as a panel consensus
- **On polarity `gemma-4-31b-it` sits closest to Luna** (κ 0.50) and reads far more articles as `Neutre` (7,275) than Luna does (5,017), so where the two agree the reading is comparatively well founded and where Gemma alone says `Neutre` it is usually reading descriptive coverage as neutral rather than positive
...
- Pass `model="all"` before quoting any single number: where the four models split, no one model's figure should be reported alone
