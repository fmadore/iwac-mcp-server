...
Three models scored the corpus independently: **`gpt-5-6-luna`**, **`mistral-small-2603`** and **`deepseek-v4-flash-0731`**. Coverage is near-total but not complete — 12,305 of 12,356 articles on polarity and centrality — so compare `scored_by_all` against `total_articles` rather than assuming they match. The ~51 missing articles are the Ewé / Kabiyè / Dendi / untagged ones, skipped on purpose because the annotation prompt is French and a French-prompted model returns confident but unusable output for them. `get_sentiment_distribution` reaches all three (and `model="all"` reports their agreement); the inline `polarity` / `centrality` / `subjectivity` columns on search results, and `search_by_sentiment`'s filters, are `gpt-5-6-luna`'s only.

**Always name the model behind a reported figure.** The columns name the exact model that produced them, and so do the tools; vendor shorthand (`chatgpt`, `mistral`, `deepseek`) is accepted as input and resolves to the model id, which is what the payload echoes back.

**Generation 1 is gone from these tools.** An earlier campaign (`gemini-3-flash-preview`, `gpt-5-mini`, `ministral-14b-2512`) scored the same corpus and its columns survive on Hugging Face, but the server no longer serves them: asking for one returns an error rather than substituting the same vendor's generation-2 model. The two generations differ in model, in prompt AND in how subjectivity is stored, so a figure from one is not comparable to a figure from the other — if a source quotes a generation-1 number, say which generation it came from rather than presenting it alongside these.
...
- **Subjectivity is much the weakest of the three scales.** Measured on the generation-2 pilot (2026-07-29): pairwise κ **0.093–0.470** — `mistral-small-2603` against `deepseek-v4-flash-0731` scores 0.093, which is chance — and DeepSeek reproduced its own answer on a re-run only **47%** of the time. These are single-run annotations, so that variance is baked into the stored value. Report it as weak evidence, with the caveat, or not at all. Polarity and centrality are far stronger. `get_sentiment_distribution` ships the caveat in the payload; quote it
- **Never align the models by their subjectivity gaps — condition on centrality.** `gpt-5-6-luna` omits a subjectivity label exactly where it said `Non abordé`; `deepseek-v4-flash-0731` omits on ~489 articles, 18 of which it did say discuss Islam. So a row where Luna is empty and Mistral says `Très objectif` is not a disagreement about objectivity — it is the two disagreeing about whether the article concerns Islam at all
- **On centrality the Mistral family is a systematic outlier** (`mistral-small-2603` runs κ 0.244–0.270 pairwise where non-Mistral pairs run 0.511–0.725) — a persistent family reading of the construct, not a bad run. So a 2-of-3 majority on centrality is almost always the other two outvoting Mistral: describe it that way rather than as a three-model consensus
